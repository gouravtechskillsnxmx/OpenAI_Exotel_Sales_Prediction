...
# ---------------------------------------------------------
# WhatsApp follow-up after call (additive only)
# ---------------------------------------------------------
...
def whatsapp_followup_enabled() -> bool:
    return bool(
        SEND_WHATSAPP_AFTER_CALL
        and EXO_API_KEY
        and EXO_API_TOKEN
        and EXO_SID
        and EXOTEL_WHATSAPP_FROM
    )


def build_post_call_whatsapp_text(*, phone_number: str = "", summary: str = "") -> str:
    phone_label = phone_number or "your number"
    summary_clean = " ".join((summary or "").split())
    if len(summary_clean) > 280:
        summary_clean = summary_clean[:277].rstrip() + "..."
    base = (
        "Namaste. Thank you for taking the call. "
        "I am sharing a quick follow-up here on WhatsApp. "
    )
    if summary_clean:
        return (
            f"{base}Mobile: {phone_label}. "
            f"Call note: {summary_clean} "
            "Reply here if you want a quick insurance review."
        )
    return (
        f"{base}Mobile: {phone_label}. "
        "Reply here if you want a quick insurance review."
...
async def send_exotel_whatsapp_message(
    *,
    to_number: str,
    body_text: str,
    ...
    """
    Send a WhatsApp message via Exotel.
    Uses the official v2 /messages endpoint. Free-form text works only in an
    open WhatsApp session; otherwise configure template env vars so this helper
    sends a template message instead. Official Exotel docs document the
    POST /v2/accounts/<sid>/messages pattern and the Mumbai cluster domain
    api.in.exotel.com.
    """
    if not whatsapp_followup_enabled():
        return {
            "ok": False,
            "reason": "whatsapp_followup_disabled_or_env_missing",
        }
    ...
    if EXOTEL_WHATSAPP_STATUS_CALLBACK:
        payload["status_callback"] = EXOTEL_WHATSAPP_STATUS_CALLBACK
        payload["whatsapp"]["messages"][0]["status_callback"] = EXOTEL_WHATSAPP_STATUS_CALLBACK

    if EXOTEL_WHATSAPP_TEMPLATE_NAME:
        payload["whatsapp"]["messages"][0]["content"] = {
            "type": "template",
            "template": {
                "name": EXOTEL_WHATSAPP_TEMPLATE_NAME,
                "language": EXOTEL_WHATSAPP_TEMPLATE_LANGUAGE,
            },
        }
    else:
        payload["whatsapp"]["messages"][0]["content"] = {
    ...
    try:
        ...
        while True:
            ...
            if ev == "connected":
                ...
                try:
                    if meta_phone:
                        whatsapp_text = build_post_call_whatsapp_text(
                            phone_number=meta_phone,
                            summary=summary_text,
                        )
                        wa_result = await send_exotel_whatsapp_message(
                            to_number=meta_phone,
                            body_text=whatsapp_text,
                            custom_data=f"call_id={meta_call_id}",
                        )
                        logger.info("Post-call WhatsApp result for %s: %s", meta_phone, wa_result)
                    else:
                        logger.info("Skipping post-call WhatsApp; no phone number available")
                except Exception:
                    logger.exception("Failed to send post-call WhatsApp")
